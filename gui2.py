...
class ProxySettingsDialog(QDialog):
    ...
    def load_proxy_from_file(self, file_path):
        """Load ALL proxies from file without auto-closing or auto-applying."""
        ...
        try:
            ...
            if proxies:
                # SIMPAN LIST PROXY KE PARENT (tanpa apply)
                self.parent.custom_proxies = proxies
                self.parent.current_proxy_index = 0

                # TAMPILKAN PROXY PERTAMA SEBAGAI PREVIEW
                self.host_input.setText(proxies[0]["server"])
                self.port_input.setText(str(proxies[0]["port"]))
                self.type_combo.setCurrentText(proxies[0]["type"].upper())

                self.parent.append_output(f"[green]Loaded {len(proxies)} proxies from {file_path}[/]")

            else:
                self.parent.append_output("[yellow]No proxies found in file[/]")

        except Exception as e:
            self.parent.append_output(f"[red]Failed to load proxy file: {e}[/]")
    ...
    def apply_proxy(self):
        ...
        if proxy_config:
            # MODE FILE PROXY
            self.parent.framework.session["proxy_mode"] = "FileProxy"
...
class LazyFrameworkGUI(QMainWindow):
    def __init__(self):
        ...
        for cache in cache_dirs:
            try:
                shutil.rmtree(cache)
                #self.append_output(f"[bold cyan][*] Cache dihapus: {cache}[/]")
            except Exception as e:
                #self.append_output(f"[bold red][!] Gagal hapus: {cache} → {e}[/]")
                self.load_banner()
        QTimer.singleShot(2000, self.start_tor_auto_rotate)
        #QTimer.singleShot(2000, self.start_global_proxy_rotate)
    ...
    def on_module_finished(self):
        """Auto-open browser: URL > TARGET > DOMAIN > HOST (SKIP CIDR & invalid)"""
        self.run_btn.setEnabled(True)
        ...
        options = self.framework.loaded_module.get_options()

        def get_opt(key):
            opt = options.get(key, {})
            return (opt.get("value") or opt.get("default") or "").strip()
        ...
        if not url:
            self.append_output("[dim]No URL/TARGET/DOMAIN/HOST to open[/]")
            return

        original = url.strip()

        # === SKIP: CIDR (192.168.1.1/24), range, atau invalid ===
        import re

        # 1. Deteksi CIDR (IP/angka)
        if re.search(r'/[0-3]?[0-9]$', original):
            self.append_output(f"[dim]CIDR detected ({original}) → skipped browser[/]")
            return

        # 2. Deteksi range (192.168.1.1-100)
        if '-' in original and re.match(r'^[\d\.]+-\d+$', original.split('/')[-1]):
            self.append_output(f"[dim]Range detected ({original}) → skipped browser[/]")
            return

        # 3. Jika sudah ada protocol → buka langsung
        if original.startswith(("http://", "https://", "file://")):
            final_url = original
        else:
            # 4. Jika IP tunggal → buka dengan http (browser akan handle)
            if re.match(r'^(\d{1,3}\.){3}\d{1,3}$', original):
                final_url = "http://" + original
            # 5. Jika domain → https
            elif re.match(r'^[a-zA-Z0-9][a-zA-Z0-9\-\.]*[a-zA-Z0-9]$', original):
                final_url = "https://" + original
            else:
                self.append_output(f"[dim]Invalid URL format ({original}) → skipped[/]")
                return

        # === BUKA DI BROWSER ===
        QTimer.singleShot(1000, lambda: self.open_in_browser(final_url))

    def open_in_browser(self, url):
        """Buka URL di tab Browser (otomatis)"""
        if not url:
            return
        ...
        # Pindah ke tab Browser
        self.tabs.setCurrentIndex(0)  # Console = 0, Options = 1, Module Info = 2 → Tapi browser ada di sidebar!
        # Wait, browser ada di LEFT SIDEBAR! Bukan di tab utama.

        # === GANTI KE BROWSER DI SIDEBAR KIRI ===
        # Cari index tab "Browser" di info_browser_tabs
        info_browser_tabs = self.findChild(QTabWidget, "")  # Kita cari manual
        # Karena tidak diberi objectName, kita cari dari parent

        # Alternatif: langsung akses self.browser (sudah ada)
        self.browser.setUrl(QUrl(url))
        self.url_bar.setText(url)

        # Pindah ke tab "Browser" di sidebar kiri
        # Cari parent tab widget di sidebar
        sidebar = self.findChild(QWidget, "")  # Tidak ada nama
        # Gunakan cara aman: cari QTabWidget di dalam sidebar
        for tab_widget in self.findChildren(QTabWidget):
            if "Browser" in [tab_widget.tabText(i) for i in range(tab_widget.count())]:
                tab_widget.setCurrentIndex(1)  # Index 1 = Browser
                break

        self.append_output(f"[bold cyan][*] Opening in browser: {url}[/]")
